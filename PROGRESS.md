...
| Field | Value |
|---|---|
| **Current phase** | **Phase 4 — Coordination** ~COMPLETE. Shipped: leader election (P4.1–3), cron + CronPanel UI (P4.4/P4.6), chaos script (P4.5), split-brain docs (P4.7), README coordination section (P4.8). Only P4.9 (demo GIFs — Kripa prefers static, effectively dropped) + P4.10 (stretch: task dependency chains) remain. Phase 3 ✅ complete. |
| **Current task** | Phase 4 P4.7 + P4.8 docs BUILT 2026-08-16 — honest "leader election & split-brain" section in `docs/ARCHITECTURE.md`; README threaded with coordination features + `docs/images/{leader,cron}.png` static shots + updated diagram/config/API/quick-start/structure. Ready to commit on `feature/p3-blocking-pickup`. |
...
| P3.7 | ☑ Benchmarks: before/after poll-vs-blocking; sustained soak asserting zero lost tasks; record machine specs | done | **3 soaks 2026-08-16, all zero-loss (Redis ground truth):** steady 40/s·20m (47,999 == 47,784 + 215), chaos 40/s·8m kill+restore (19,199 == 19,116 + 83; reaper reclaimed 9 dead-node leases), sustained-overload 250/s·45m (132,385 accepted == 131,883 + 502; 542,613 shed by backpressure, 0 errors). poll-vs-blocking already recorded (P3.4). Gotcha found: default `SOAK_DRAIN_TIMEOUT=300s` trips a false FAIL when draining a 20k backlog — raise it for overload runs (documented in BENCHMARKS) |
| P3.8 | ☑ Docs: `docs/BENCHMARKS.md` — throughput, p50/p99 enqueue-to-start latency, zero-loss chaos results | done | **Filled 2026-08-16.** Throughput ceiling (3 workers): hash ~580/s (CPU), sleep ~21/s (slot-bound), mix ~49/s (sleep is the cap). task_duration p50/p95/p99 per type. Pickup latency honest across regimes (sub-capacity median ~3ms; near-cap/overload p99 → seconds = queue-wait, not a doorbell regression; isolated doorbell micro-bench p99 ~0.86ms stands). Corrected reproduce commands + machine-relative rate guidance |
...
- [x] BENCHMARKS.md has honest numbers with machine specs and caveats
...
| P4.4 | ☑ Cron jobs: `POST /api/cron` (spec + task template) stored in `taskqueue:cron`; leader materializes due instances. Use `robfig/cron` parser. Instance IDs = `{cronID}-{scheduledTs}` for failover dedup (reuses P1.6) | done | New `internal/cron/` (cron.go/store.go/materializer.go); `robfig/cron/v3` `SecondOptional` (5- & 6-field). Materializer runs inside `RunWhenLeader` (leaderCtx); **skip-to-latest** catch-up; Exists-gated enqueue then cursor-last. `CRON_TICK_MS` (1000). POST/GET/DELETE `/api/cron`. **Frontend `CronPanel`** (create form w/ schedule presets + plain-English translation + task-type toggle; live job list w/ enabled badge, last-fired, delete) on the playground — screenshot-verified desktop+mobile, console clean. **Live-verified:** */10s job fired 43 slots contiguous, zero duplicates, across a leader kill+failover (crown scheduler-1→2, cron kept firing on new leader) |
| P4.5 | ☑ Chaos script: random worker/broker kills for 10 min → invariant check (`submitted == completed + dead_lettered`, no duplicate cron fires) | done | `scripts/chaos.sh` + `make chaos` (in backend/Makefile). Repeated random kills of workers (→reaper reclaim) AND schedulers (→leader failover) under load with a cron job running; keeps ≥1 of each alive; killed nodes auto-revive via `restart: unless-stopped`. Asserts zero-loss + cron-fired + drain from Redis ground truth; env-driven (CHAOS_DURATION 5m default). **Live-verified:** 90s run, 3 kills + 1 leader migration, reaper reclaimed 10 leases, cron fired 10 slots, **PASS zero-loss 3609 == 3598 + 11**, queues drained |
...
| P4.7 | ☑ Docs: "Leader election & split-brain" — honest section on the unsafety window without fencing tokens, why lease-not-Raft | done | New **"Coordination: leader election, cron & split-brain (Phase 4)"** section in `docs/ARCHITECTURE.md` (replaced the stale "planned Phases 3–4"): lease protocol, RunWhenLeader gating, cron dedup, the ~TTL two-leader window + why it's safe (atomic promote/reclaim + idempotency), the fencing-token/slow-leader caveat, the cron Save-then-Enqueue window; updated Mermaid to the API-only-backend + scheduler topology |
| P4.8 | ☑ README: leader-election/cron/chaos section, diagrams, delivery-guarantees link | done | Threaded through README (STATIC screenshots per Kripa — no GIFs): new Coordination feature group; 👑 leader + cron `docs/images/{leader,cron}.png`; updated ASCII architecture diagram; config rows (LEADER_ELIGIBLE/LEADER_TTL_MS/CRON_TICK_MS); API rows (/api/leader, /api/cron); quick-start failover + cron + `make chaos` steps; project structure (election/cron/chaos.sh); link to the split-brain doc. (Roadmap section stays removed per Kripa) |
...
| 2026-08-15 | **P3.6 backpressure sheds at the ready-queue depth only, checked before any write** (not ready+delayed+processing; not a token-bucket rate limit) | Ready depth is the backlog-pressure signal that matches the `queue_depth` metric; delayed tasks are future-scheduled (not pressure) and processing is bounded by worker capacity. Checking before persist/enqueue means a rejected submit leaves zero trace (at-least-once intact) and sheds load cheaply (one O(1) ZCARD). Depth-based shedding (not rate limiting) is what the task specifies; `MAX_QUEUE_DEPTH=0` preserves the old unbounded behavior |
...
### 2026-08-16 — Phase 4 P4.7 (split-brain docs) + P4.8 (README coordination section)
- **P4.7 — `docs/ARCHITECTURE.md`:** replaced the stale "Target architecture (planned — Phases 3–4)" with a shipped **"Coordination: leader election, cron & split-brain (Phase 4)"** section. Documents the lease protocol (acquire/renew CAS/step-down/release), `RunWhenLeader` gating, cron materialization + `{cronID}-{ts}` idempotency + skip-to-latest, and **the honest split-brain analysis**: the ~TTL two-leader window is safe because promote/reclaim are atomic + cron dedups; a lease can't fence a *slow* old leader (blast radius = one at-least-once reprocess, already permitted) — lease-not-Raft is a conscious tradeoff, fencing tokens named as the fix; plus the cron Save-then-Enqueue sub-second loss window. Updated the Mermaid to the API-only-backend + scheduler-replicas topology.
- **P4.8 — `README.md`** (STATIC screenshots per Kripa's earlier call — no GIFs): new **Coordination (leader election + cron)** feature group; a "👑 Coordination" screenshots subsection with `docs/images/leader.png` (crown on the leader) + `docs/images/cron.png` (CronPanel); rewrote the ASCII architecture diagram (backend API-only + leader-elected scheduler + leader lease); added config rows (`LEADER_ELIGIBLE`, `LEADER_TTL_MS`, `CRON_TICK_MS`), API rows (`/api/leader`, `/api/cron`), quick-start steps (scheduler scale, kill-the-leader failover, create-a-cron, `make chaos`), project-structure entries (`internal/election`, `internal/cron`, `scripts/chaos.sh`), and a link to the split-brain doc. Roadmap section stays removed (Kripa).
- **Screenshots** captured via Playwright element-screenshots of the live panels; both verified. Links/anchors/image refs all resolve; no stale "planned Phases 3" / "API·scheduler·reaper" text left.
- **STATE:** UNCOMMITTED on `feature/p3-blocking-pickup`: M `README.md`, M `docs/ARCHITECTURE.md`, M `PROGRESS.md`, new `docs/images/{leader,cron}.png`. Docs only — build/test gate unaffected. Kripa commits. **Phase 4 essentially done** (remaining: P4.9 GIFs dropped, P4.10 stretch dependency chains).

---

### 2026-08-16 — Phase 4 Increment 3: Chaos Script (P4.5)
- **★ approved** (5 min default, ~20s kill interval). Built `scripts/chaos.sh` + a `chaos` target in `backend/Makefile`.
- **What it does:** ensures the stack up, flushes, registers a cron job, drives `loadgen` for the duration, then a chaos loop kills a RANDOM worker (→ reaper reclaim) or scheduler (→ leader failover) every `CHAOS_INTERVAL`, **never dropping below 1 running worker or 1 scheduler**. Killed containers auto-revive via compose `restart: unless-stopped`. Tracks leader migrations. After load: deletes the cron, restores scale, waits for drain, and asserts from Redis ground truth — **zero loss** (`submitted == processed + failed`, also catches double-processing), **cron fired ≥1 slot**, **queues drained**. PASS/FAIL + non-zero exit. Env-driven (`CHAOS_DURATION/INTERVAL/RATE/MIX/WORKERS/SCHEDULERS/CRON/DRAIN_TIMEOUT`).
- **Cron invariant note:** "no duplicate fires" is guaranteed structurally by the `{cronID}-{ts}` idempotent IDs and confirmed by the zero-loss accounting; gaps *can* occur during a leaderless failover window (skip-to-latest, by design) so the script asserts no-duplicate/no-loss, not no-gap.
- **Portability:** bash 3.2 compatible (no `mapfile`); `docker compose ps --status running` for the keep-alive floor.
- **Live-verified (90s run, interval 18s):** 3 kills (2 worker, 1 scheduler), 1 leader migration, reaper reclaimed 10 leases, cron fired 10 slots, **✔ PASS zero-loss 3609 == 3598 + 11**, queues drained. `bash -n` clean, `make chaos` resolves.
- **STATE:** UNCOMMITTED on `feature/p3-blocking-pickup`: new `scripts/chaos.sh`, M `backend/Makefile`, M `PROGRESS.md`. No Go/compose changes → build/test gate unaffected. Kripa commits. **Next: P4.7 docs** (split-brain window + at-least-once honesty) → P4.8 README leader-election section.
...
- **Cron backend committed+pushed by Kripa.** Follow-up (this session): added the **CronPanel frontend** (create/list/delete cron jobs from the playground; schedule presets so no cron syntax needed; plain-English "fires every N seconds" translation; educational leader explainer). Followed the frontend-ui skill: `tsc` + `next build` clean, screenshot-verified desktop (1440) + mobile (390), empty/populated states, console zero errors; create flow driven end-to-end through the UI. UNCOMMITTED: new `frontend/components/CronPanel.tsx`, M `frontend/{app/playground/page.tsx, lib/api.ts, lib/types.ts}`.
...
### 2026-08-16 — Phase 3 CLOSED: P3.7 soak runs + P3.8 BENCHMARKS.md (measurement)
- **Ran 3 soaks (3 workers, `--scale worker=3`, mix hash:50/sleep:50), all zero-loss from Redis ground truth:**
  - **Steady-state** 40/s · 20 min → PASS: 47,999 accepted == 47,784 completed + 215 dead-lettered, 10,064 retries, 4 reclaims, queues drained.
  - **Chaos** 40/s · 8 min, kill + restore one worker mid-run → PASS: 19,199 == 19,116 + 83; the reaper reclaimed the dead node's **9** in-flight leases and they retried to completion — the kill-a-worker demo proven loss-free.
  - **Sustained overload** 250/s · 45 min → zero-loss confirmed (132,385 accepted == 131,883 + 502); the queue pinned at `MAX_QUEUE_DEPTH=20000` and backpressure shed **542,613** submissions as `429 + Retry-After`, **0 errors**.
- **Throughput ceiling measured (`cmd/loadgen` saturation):** `hash` (CPU-bound, 100k rounds) **~580/s** (peak 612); `sleep` (500 ms) **~21/s** (concurrency-slot bound, ~10–11 slots); 50/50 mix **~49/s** (the `sleep` half caps it). ~27× spread — the reason 40/s is loss-free and 250/s is backpressure.
- **Latency:** `task_duration` p50/p95/p99 — hash 21.8/47.9/74.9 ms, sleep ~0.75/0.98/0.99 s. `enqueue_to_start` sub-capacity median ~3 ms; near-capacity/overload p99 → seconds (queue-wait, Little's law), **not** a doorbell regression — isolated P3.4 doorbell micro-bench p99 ~0.86 ms stands.
- **Filled `docs/BENCHMARKS.md`** results (soak + throughput + latency tables, honest caveats); **corrected the reproduce block** (250/s is the overload/backpressure regime, not a "5-min validation"; steady = 40/s; machine-relative rate guidance).
- **Gotcha found & documented:** `SOAK_DRAIN_TIMEOUT` default 300 s trips a *false* FAIL on the 45 min/250 run — draining a full 20k queue at ~49/s takes ~7 min. Raise the timeout for overload runs; the zero-loss invariant itself held (verified by manual drain to `ready=0`).
- **Verified:** no Go changes → build/test gate unaffected; all numbers from live Prometheus + Redis. Docker stack left **up**.
- **STATE:** UNCOMMITTED on `feature/p3-blocking-pickup`: `M docs/BENCHMARKS.md`, `M PROGRESS.md`. Kripa to commit + MR. **Next: Phase 4** (leader election via Redis lease, gate scheduler/reaper/cron behind leadership, cron jobs, README rewrite with demo GIFs).
...
### 2026-08-15 — Phase 3 P3.6 backpressure (~1.5h)
- **Done:** Depth-based load shedding on `POST /api/tasks`. New config `MAX_QUEUE_DEPTH` (0 = disabled, backward-compatible) + `RETRY_AFTER_SECONDS` (default 5), validated. `SubmitTask` checks `QueuePeek.ReadySize` (ZCARD ready) right after JSON decode and **before any write**; at/above the cap it returns `429 Too Many Requests` + `Retry-After` header + a JSON body (`error`, `queue_depth`, `max_queue_depth`, `retry_after_seconds`), so a shed submit leaves no record/event/enqueue (at-least-once untouched). New Prometheus `tasks_rejected_total{reason}` counter (threaded `telemetry.Metrics` into `HandlerDeps`, wired from `cmd/server`) + a new Grafana panel "Rejected / Backpressure (per sec)". compose `backend` env defaults `MAX_QUEUE_DEPTH=20000`, `RETRY_AFTER_SECONDS=2`.
- **Verified:** `go build/vet/gofmt` clean, `go test -race` all green (new `TestSubmitBackpressure` = cap 2 → 3rd submit 429 + `Retry-After: 3` + ready stays 2; config default/override/validation tests). **Live** (rebuilt image, cap=20000, 3 workers): `loadgen -rate 2000 -duration 45s -mix sleep:100` → intake accepted ~20K then **plateaued at the cap**, **68,997 rejected (429), 0 errors**; Prometheus `tasks_rejected_total=68997`, `queue_depth` peak **20,009** (vs 26K runaway in P3.5 without backpressure). Live probe over the cap returned `HTTP 429` + `Retry-After: 2` + the JSON body. Grafana screenshot: 11 panels, 0 errors, queue-depth flat-top + rejected/s trapezoid.
- **Learned/decided:** shed on ready depth only, before writes (see Decision Log 2026-08-15); it's depth-based shedding, not rate limiting. One extra O(1) ZCARD per submit — negligible. loadgen's built-in `http_429` counter made the demo one command.
- **Next:** P3.7 (benchmarks: poll-vs-blocking, 30-min soak asserting `submitted == completed + dead_lettered`, record machine specs) → P3.8 (BENCHMARKS.md write-up). Then Phase 4 (leader election, cron).
- **Blockers:** none.
